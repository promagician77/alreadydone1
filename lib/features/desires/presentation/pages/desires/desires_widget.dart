...
class _DesiresWidgetState extends State<DesiresWidget> { 
  ...
  Future<void> _loadData() async {
    try {
      ...
      try {
        final list = await desiresRepository.getDesires();
        print('list: $list');
  ...
  Future<void> _performDelete(_StoryItem story) async {
    ...
    try {
      await desiresRepository.deleteStory(storyId);
      if (!mounted) return;
  ...
  /// Non-subscribers subscribe in the upsell sheet; creating stories requires a subscription.
  Future<void> _handleAddNewManifestation() async {
    ...
    if (!subscribed) {
      ...
      if (!mounted || outcome != SubscriptionUpsellOutcome.subscribed) return;
    }
