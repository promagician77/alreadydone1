...
class _PlayerWidgetState extends State<PlayerWidget>
  ...
  Future<void> _loadStoryData() async {
    ...
    if (widget.storyId == null) {
      ...
      if (userId != null) {
        try {
          ...
          if (storiesList.isEmpty) {
            await playerRepository.clearLastPlayed();
            if (!mounted) return;
            setState(() {
              _hasNoStory = true;
              _loading = false;
      ...
      final lastPlayed = await playerRepository.loadLastPlayed();
      if (lastPlayed != null &&
      ...
      if (fallbackMap != null) {
        ...
        playerRepository.saveLastPlayed(
    ...
    try {
      ...
      if ((storyVoiceId == null || storyVoiceId.isEmpty) &&
        ...
        if (userId != null && mounted) {
          try {
            final storiesRes = await playerRepository.getStories(userId);
  ...
  Future<void> _onVoiceChanged(String? selectedValue) async {
    ...
    try {
      final res = await playerRepository.voiceGenerateAudio(
        voiceId: voiceIdToUse,
        storyId: storyId,
      );
  ...
  Future<void> _deepenManifestation() async {
    ...
    try {
      final res = await playerRepository.deepenStory(
        userId: userId,
        storyId: storyId,
        name: name,
        location: location,
      ...
      if (voiceIdToUse != null && voiceIdToUse.isNotEmpty) {
        try {
          final audioRes = await playerRepository.voiceGenerateAudio(
            voiceId: voiceIdToUse,
