...
import { Settings, Play, ChevronLeft, Sparkles, SkipBack, SkipForward, Rewind, FastForward } from 'lucide-react';

export default function DoneLibraryCoachmark() {
    ...
    <div style={{ minHeight: '100vh', background: '#2a241c', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: c.serif, gap: '40px', flexWrap: 'wrap' }}>
      ...
      <div style={{ width: '380px', height: '780px', background: '#000', borderRadius: '48px', padding: '12px', boxShadow: '0 30px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.06)', position: 'relative', flexShrink: 0 }}>
        <div style={{ width: '100%', height: '100%', background: c.bg, borderRadius: '38px', overflow: 'hidden', position: 'relative' }}>
          ...
          <div style={{ padding: '0 20px', position: 'relative', height: 'calc(100% - 40px)', overflow: 'hidden' }}>
            ...
            {/* Bottom nav with Done highlighted */}
            <div
              style={{
                position: 'absolute',
                bottom: '0',
            ...
            >
              {[
                ...
                { icon: '✓', label: 'Done', active: false, target: true },
                ...
                <div
                  ...
                  style={{
                    ...
                    alignItems: 'center',
                    gap: '4px',
                    padding: !dismissed && item.target ? '6px 14px' : '0',
                    borderRadius: '12px',
                ...
                >
                  <span style={{ fontSize: '18px', color: item.active || (!dismissed && item.target) ? c.gold : c.text, fontWeight: item.active || (!dismissed && item.target) ? 700 : 400 }}>{item.icon}</span>
                  <span style={{ fontSize: '11px', color: item.active || (!dismissed && item.target) ? c.gold : c.text, fontFamily: c.sans, fontWeight: item.active || (!dismissed && item.target) ? 700 : 500 }}>{item.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {!dismissed && (
            ...
            >
              {/* Arrow pointing down to Done tab */}
              <div style={{
                position: 'absolute',
                bottom: '-10px',
                left: '50%',
      ...
      <div style={{ maxWidth: '320px', color: c.bg }}>
        ...
        <div style={{ fontSize: '32px', fontFamily: c.serif, fontWeight: 500, color: c.bg, lineHeight: 1.05, marginBottom: '8px' }}>
          Library <span style={{ fontStyle: 'italic', color: c.goldLight }}>Tip</span>
        </div>
        <div style={{ fontSize: '13px', color: '#a89b85', fontFamily: c.sans, lineHeight: 1.5, marginBottom: '24px' }}>
          One-time popup pointing to the Done tab in the bottom navigation, where all manifestations are stored.
        </div>
        ...
        <div style={{ fontSize: '11px', color: '#a89b85', fontFamily: c.sans, lineHeight: 1.6 }}>
          ...
          • Triggers after first manifestation is saved<br />
          • Dismisses on "Got it" or tapping the Done tab<br />
          • Background dims to draw focus<br />
          • Arrow points down to highlighted Done tab
        </div>
      </div>
